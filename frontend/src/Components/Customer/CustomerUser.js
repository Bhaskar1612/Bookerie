      ...
      {books.map(book => (
        <div className="book-item" key={book['Book_ID']}>
          ...
          <div className="book-actions">
            <label className="book-quantity-label" htmlFor={`quantityInput-${book['Book_ID']}`}>Quantity:</label>
            <input
              className="book-quantity-input"
              id={`quantityInput-${book['Book_ID']}`}
              type="number"
              value={quantity[book['Book_ID']] || 0}
              onChange={(e) => handleQuantityChange(book['Book_ID'], e.target.value)}
            />
            <button className="add-to-cart-button" onClick={() => handleAddToCart(book['Book_ID'], book['Book_Name'])}>Add to Cart</button>
          </div>
        </div>
      ))}
    </div>
